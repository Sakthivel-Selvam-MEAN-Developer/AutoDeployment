...
describe('Overall Trip model', () => {
    ...
    test('should able to get only overall trip by id', async () => {
        ...
        expect(actual?.loadingPointToUnloadingPointTripId).toBe(
            trip1.loadingPointToUnloadingPointTripId
        )
    })
    test('should able to close overall trip by changing Acknowledgement Status to true', async () => {
        const loadingPricePointMarker = await createPricePointMarker(seedPricePointMarker)
        const stockPricePointMarker = await createPricePointMarker({
            ...seedPricePointMarker,
            location: 'salem'
        ...
        const actual = await closeAcknowledgementStatusforOverAllTrip(trip1.id)
        expect(actual.acknowledgementStatus).toBe(true)
    })
})
